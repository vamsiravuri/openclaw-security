...
if [ -n "$EXISTING" ]; then
    ok "Nightly cron already registered -- skipping"
else
    openclaw cron add \
        ...
        --no-deliver \
        --timeout 120000 \
        --message "You are running an automated nightly security patch. Follow these steps exactly:
1. Fetch the latest skills list from https://github.com/vamsiravuri/openclaw-security/tree/release/skills
2. For each skill, compare the SKILL.md content with the currently installed version
3. If the skill is new or updated, install it by copying to the skills directory
4. Run: openclaw security audit
5. Report a summary of: skills updated, skills unchanged, audit result (critical/warn counts)
6. If any critical findings, escalate immediately via WhatsApp" 2>/dev/null || fail "Failed to register nightly cron"
    ok "Nightly security patch cron registered"
fi
